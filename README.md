...
- Swaps unstable values you declare, like timestamps and IDs, for fixed replay variables. A path segment can hold at most one variable, so `/items/{{id}}.json` works but `/items/{{a}}-{{b}}` doesn't
